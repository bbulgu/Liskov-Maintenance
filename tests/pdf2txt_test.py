...
def test_write_file():
    pdf2txt.main(['pdf2txt.py', '-o',
                  'tests/results/result1.txt', 'samples/simple1.pdf'])
    with open('tests/results/result1.txt') as outfile_new:
        lines_new = outfile_new.readlines()
    oldpdf2txt.main(['pdf2txt.py', '-o',
                     'tests/results/result2.txt', 'samples/simple1.pdf'])
    with open('tests/results/result2.txt') as outfile_old:
