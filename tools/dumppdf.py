...
def dumpxml(out, obj, mode=None):
    ...
    if isinstance(obj, bytes):
        out.write(
            '<string size="%d">%s</string>' % (len(obj), encode(obj))
        )
        return
            ...
            for (level, title, dest, a, se) in outlines:
                ...
                if dest:
                    ...
                    if isinstance(action, dict):
                        ...
                        if (
                            subtype
                            and repr(subtype) == "/GoTo"
                            and action.get("D")
                        ):
                            dest = resolve_dest(action["D"])
                            pageno = pages[dest[0].objid]
                outfp.write(
                    '<outline level="%r" title="%s">\n'
                    % (level, q(s))
                )
                if dest is not None:
                    outfp.write("<dest>")
...
def extractembedded(
    ...
    def extract1(obj):
        ...
        if fileobj.get("Type") is not LITERAL_EMBEDDEDFILE:
            raise PDFValueError(
                "unable to process PDF: reference \
                for %r is not an EmbeddedFile"
                % (filename)
            )
