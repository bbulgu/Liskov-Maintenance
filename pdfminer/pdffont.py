...
class CFFFont:
    ...
    def __init__(self, name, fp):
        ...
        if format == b"\x00":
            ...
            raise ValueError(
                "unsupported encoding format: %r" % format
            )
        # Charsets
        self.name2gid = {}
        self.gid2name = {}
        ...
        if format == b"\x00":
            ...
            raise ValueError(
                "unsupported charset format: %r" % format
            )
        # print(self.code2gid)
        # print(self.name2gid)
        # assert 0
...
class TrueTypeFont:
    ...
    def __init__(self, name, fp):
        ...
        for _ in range(ntables):
            (name, tsum, offset, length) = struct.unpack(
                ">4sLLL", fp.read(16)
            )
    ...
    def create_unicode_map(self):
        ...
        for (_1, _2, st_offset) in subtables:
            ...
            (fmttype, fmtlen, fmtlang) = struct.unpack(
                ">HHH", fp.read(6)
            )
            if fmttype == 0:
                char2gid.update(
                ...
                (segcount, _1, _2, _3) = struct.unpack(
                    ">HHHH", fp.read(8)
                )
                segcount //= 2
                ecs = struct.unpack(
                    ">%dH" % segcount, fp.read(2 * segcount)
                )
                fp.read(2)
                scs = struct.unpack(
                    ">%dH" % segcount, fp.read(2 * segcount)
...
class PDFFont:
    def __init__(self, descriptor, widths, default_width=None):
        ...
        self.widths = widths
        self.fontname = resolve1(
            descriptor.get("FontName", "unknown")
        )
...
class PDFSimpleFont(PDFFont):
    def __init__(self, descriptor, widths, spec):
        ...
        if "ToUnicode" in spec:
            ...
            CMapParser(
                self.unicode_map, BytesIO(strm.get_data())
            ).run()
...
class PDFType1Font(PDFSimpleFont):
    def __init__(self, rsrcmgr, spec):
        ...
        try:
            (descriptor, widths) = FontMetricsDB.get_metrics(
                self.basefont
            )
        except KeyError:
            ...
            widths = dict(
                (i + firstchar, w) for (i, w) in enumerate(widths)
            )
...
class PDFType3Font(PDFSimpleFont):
    def __init__(self, rsrcmgr, spec):
        ...
        (self.hscale, self.vscale) = apply_matrix_norm(
            self.matrix, (1, 1)
        )
        return
...
class PDFCIDFont(PDFFont):
    def __init__(self, rsrcmgr, spec):
        ...
        registry = bytes_value(
            self.cidsysteminfo.get("Registry", b"unknown")
        )
        ordering = bytes_value(
            self.cidsysteminfo.get("Ordering", b"unknown")
        )
        ...
        if "FontFile2" in descriptor:
            ...
            ttf = TrueTypeFont(
                self.basefont, BytesIO(self.fontfile.get_data())
            )
        self.unicode_map = None
        if "ToUnicode" in spec:
            strm = stream_value(spec["ToUnicode"])
        ...
        if self.vertical:
            ...
            widths = dict(
                (cid, w) for (cid, (w, _)) in widths.items()
            )
            default_width = w
        else:
        ...
        PDFFont.__init__(
            self, descriptor, widths, default_width=default_width
        )
        return
