...
class PDFLayoutAnalyzer(PDFTextDevice):
    ...
    def begin_figure(self, name, bbox, matrix):
        ...
        self.cur_item = LTFigure(
            name, bbox, mult_matrix(matrix, self.ctm)
        )
        return
    ...
    def paint_path(self, gstate, stroke, fill, evenodd, path):
        ...
        if shape == "ml":
            ...
            if x0 == x1 or y0 == y1:
                self.cur_item.add(
                    LTLine(gstate.linewidth, (x0, y0), (x1, y1))
                )
                return
        if shape == "mlllh":
            # rectangle
            ...
            if (x0 == x1 and y1 == y2 and x2 == x3 and y3 == y0) or (
                ...
                self.cur_item.add(
                    LTRect(gstate.linewidth, (x0, y0, x2, y2))
                )
                return
        # other shapes
        pts = []
        for p in path:
            for i in range(1, len(p), 2):
                pts.append(
                    apply_matrix_pt(self.ctm, (p[i], p[i + 1]))
                )
...
class HTMLConverter(PDFConverter):
    ...
    def begin_div(
        self, color, borderwidth, x, y, w, h, writing_mode=False
    ):
        self._fontstack.append(self._font)
        self._font = None
        self.write(
            '<div style="position:absolute; \
            border: %s %dpx solid; writing-mode:%s; '
